...
Read the existing test suite and changed code paths. Identify the highest-risk missing regression tests for currently implemented behavior and for the bug fixes under consideration.
...
This command runs in **whole-repo audit mode** — it writes into `audit-findings.md` against the repo's `Scope` (`whole-repo`, `package: <name>`, or `paths: [<list>]`).

For per-story reviews of a single feature's changed surface, use `/review-story` instead. The auditor agent applies a stricter rubric in that mode (every AC must have a referenced test that exists and runs; every adapter must have a non-mock integration test; every implemented Gherkin `Sn` must trace to a test name) — see `~/.cursor/agents/auditor.md` "Per-story test-coverage rubric".
...
Use only `TEST-#` finding IDs for rows and headings created by this command.
...
- read the current non-`TEST-#` findings and prioritize tests that raise fix confidence for the best `fix now` candidates
