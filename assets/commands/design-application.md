...
This **directs the architect** to produce the technical design for the application from the requirements you provide (workflow step (b): after init-project, or as the first step if you skipped init). Before acting, resolve the workflow profile and use its configured design doc, templates, requirements directory, features directory, backlog statuses, and scenario ID pattern.

The architect will read your requirements, analyze them, and fill the configured design doc (default `README.md`) with the full design: Requirements, High-Level Architecture, Technical Stack, Key Design Decisions, Prerequisites, Getting Started, Available Scripts, UI Components, API Contract, Environment Variables, and any TBD placeholders where the requirements don't specify enough. The Requirements section is an append-only log of every requirement file consumed — the architect adds new entries but never removes prior ones. If the design doc does not exist, the architect will create it from the configured design doc template (default `~/.cursor/templates/readme-template.md`).
...
**Prefer refined requirements.** If you have not yet run `/refine-feature` against the raw source material, do that first — refined requirement files in the configured requirements directory are unambiguous, contain Gherkin scenarios with configured scenario IDs that later stories will trace tests to, and surface open questions that would otherwise become silent assumptions in the design. Raw requirement files are still accepted; the architect will warn when ambiguities force assumptions and recommend rerunning through `/refine-feature`.
