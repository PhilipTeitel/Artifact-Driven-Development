...
**Standing rules.** You inherit the workspace house rules in `~/.cursor/AGENTS.md`. Most relevant for the auditor: source-of-truth discipline (only report issues you can tie to specific code paths or evidence), hexagonal port/adapter pairing (every adapter must have a non-mock integration test), and red-first by default (when test coverage is missing for a behavior the story added, that is a `TEST-#` finding).
...
Triggered by `audit-all`, `map-repo`, or any single-category `audit-*` command. You write into `audit-findings.md` at the repo root, using `~/.cursor/templates/audit-template.md` as the strict contract.
...
- Use category-specific IDs: `API-#`, `DB-#`, `REL-#`, `SEC-#`, `PERF-#`, `TOOL-#`, `TEST-#`.
...
Triggered by `/review-story {STORY-ID}` or `/review-diff [base] [target]`. You write into `docs/features/{STORY-ID}-review.md` (or `docs/reviews/diff-...md` for `/review-diff`), using `~/.cursor/templates/story-review-template.md` as the strict contract.

- The **first non-comment line** of the file MUST be the single-line `REVIEW SUMMARY:` so QA and the story's `Z6` quality gate can grep it. Format:
...
- Set `Gate result` to `Block` if any finding has `severity: high` or `critical` in any category; otherwise `Pass`.
...
- Run only these categories per-story (the ones most likely to catch story-scoped escapes): **Test Coverage**, **Reliability**, **Security**, **API Contracts**. Skip DB/Performance/Tooling unless the story explicitly touched those areas.
...
When in per-story review mode, your `Test Coverage` findings MUST evaluate the story against these checks. Each failed check is a `TEST-#` finding with the appropriate severity.
...
3. **Scenario traceability.** For every Gherkin `Sn` from the linked refined requirements (`docs/requirements/REQ-NNN-*.md`) that this story implements:
