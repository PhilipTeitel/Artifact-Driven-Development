...
- A base ref to diff against. Defaults to the configured base branch candidates (defaults: `main` or `master`, auto-detected). The user can pass any ref the working tree can reach (default remote example: `origin/main`, a commit SHA, a tag).
...
5. If a story ID is provided, checks whether the diff matches one or more ledger rows (including `Change ref` and `Review ref`). Missing or stale ledger entries are reported as documentation drift, not silently ignored. When the review is for a post-complete follow-up, the ledger row's `Review ref` should point to the written configured diff-review artifact.
6. Writes findings to the configured diff-review artifact path (default `docs/reviews/diff-{base-ref-slug}--{target-ref-slug}-{YYYYMMDD}.md`) using the review template (auto-create the configured reviews directory if missing).
7. The **first line** of the output file is the configured review summary line (default `REVIEW SUMMARY:`) so the result is grep-friendly.
8. Sets `Gate result` using the configured review gate (default: `Block` on any `high`/`critical` finding; otherwise `Pass`).

## Notes

- Out-of-plan detection does not apply here (there is no story plan); instead, all changed files are in scope by definition.
- Adapter test gating still applies: a modified adapter without an integration test in the diff or existing tests is a configured test-prefix finding with the configured blocking severity (default `TEST-#` `high`).
...
- `/review-diff {git.defaultRemoteBase}` — review against the configured remote base branch
- `/review-diff v1.4.0` — review since the v1.4.0 tag
- `/review-diff {git.defaultRemoteBase} feature/{story-slug}` — review the feature branch against the configured remote base
- `/review-diff {git.defaultRemoteBase} HEAD {STORY-ID}` — review a diff in the context of a completed story follow-up
