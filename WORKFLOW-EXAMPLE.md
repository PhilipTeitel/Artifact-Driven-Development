...
The raw source idea lives in [`example/docs/requirements/llm-wiki.md`](example/docs/requirements/llm-wiki.md). The refined requirements live in [`example/docs/requirements/REQ-001-llm-wiki-cli.md`](example/docs/requirements/REQ-001-llm-wiki-cli.md).
...
That story is intentionally small. It is easier to understand the ADD trace on a small artifact before looking at a full application slice.
...
flowchart LR
    ...
    raw --> req --> design --> story --> implementation --> review --> qa --> stageGate
```

Each arrow is a handoff between artifacts. The next step does not depend on somebody remembering what the model said. It depends on a file that can be reviewed, linked, audited, and reused.
...
ADD does not ask the implementer to infer those answers from the chat. The first step is to refine the idea into requirements.
...
## 4. How The Story Artifact Was Produced
...
- The review artifact says whether the changed surface has blocking risk.
...
2. [`example/docs/requirements/REQ-001-llm-wiki-cli.md`](example/docs/requirements/REQ-001-llm-wiki-cli.md) — refined requirements and scenarios.
3. [`example/README.md`](example/README.md) — design, stack, decisions summary, and backlog.
4. [`example/docs/decisions/ADR-003-vault-wiki-layout.md`](example/docs/decisions/ADR-003-vault-wiki-layout.md) — the binding layout decision for this story.
5. [`example/docs/features/S1-1-wiki-schema-template.md`](example/docs/features/S1-1-wiki-schema-template.md) — the story spec and completion evidence.
6. [`example/docs/features/S1-1-review.md`](example/docs/features/S1-1-review.md) — the review gate.
7. [`example/docs/features/S1-6-stage1-e2e-acceptance.md`](example/docs/features/S1-6-stage1-e2e-acceptance.md) — the Stage 1 end-to-end gate.

Then read the reusable machinery:

1. [`assets/agents/architect.md`](assets/agents/architect.md)
2. [`assets/commands/plan-story.md`](assets/commands/plan-story.md)
3. [`assets/templates/user-story-template.md`](assets/templates/user-story-template.md)
