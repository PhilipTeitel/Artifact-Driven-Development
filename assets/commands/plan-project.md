...
This **directs the architect** to create the epics and stories needed to build the project (workflow step (c): after the architect has designed the app). Before acting, resolve the workflow profile and use its configured design doc, requirements directory, features directory, backlog statuses, templates, and workflow lanes.

**Sources of truth:** the configured design doc (default `README.md`, the accumulated design state) and the new requirements document(s) the user points you to. The architect reads the current design doc as the baseline — it already encodes all prior design decisions. Only the new requirement files the user provides need to be read. Old requirement files are an audit trail and do not need to be re-read unless the user explicitly includes them.

**Prefer refined requirements in the configured requirements directory** (default `docs/requirements/REQ-NNN-*.md`) produced by `/refine-feature`. Their Gherkin scenario IDs become the basis of the Test Plan in each story document, so test names trace back to user-stated behavior. Raw requirements are accepted but will produce backlog rows with weaker traceability.
