...
4. Refuse a defect that asks a system-wide question the methodology says has no system-wide answer (for example a single numeric tolerance for every path). Tell the caller to record a `DEC-NNN` that comparison is per path, and put the bound in that path's test plan.
