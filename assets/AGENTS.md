...
- **`modernization-port`** — brownfield modernization, language/framework port, or replacement of a legacy application. Use `/assess-modernization` before design, then `/plan-migration` → `/document-legacy` (scoped to the next slice's `XP-NNN` IDs) → `/refine-feature` (path details become `REQ-NNN` with section 4b provenance) and `/recover-domain` → `/design-application` or `/plan-project` → `/plan-story` → `/complete-story`. Do not skip the assessment unless the user explicitly accepts the unknown feasibility risk. Do not catalog the entire legacy system before the first slice. Do not plan or implement through a parallel port-story pipeline.
...
- **Slice prerequisites gate delivery.** Global inventories inform sequencing. They do not gate a slice. Each slice's migration-plan row, and the ADD story that implements its recovered `Sn` IDs, carries a short prerequisite table of the `DEP-NNN`, `DEC-NNN`, and `DEF-NNN` IDs that bind it. Check off resolved prerequisites on the story.
...
- **Provenance blocks readiness for the bound scope.** A requirement is not `Ready for Design`, and a story is not implementation-ready, when any covered claim remains `E4` or `E5` unless a `DEC-NNN` accepts or resolves the uncertainty. Unresolved claims on other slices do not block this one.
...
- **Comparison is per path.** Binding numeric, text, and semantic rules live on the `REQ-NNN` that covers that path (section 4b and Constraints). A workflow-profile hint is not a gate. Do not file a system-wide tolerance defect.
...
- **Parity-first is the red-first specialization.** When a story's test plan has `parity` rows, characterization tests against the REQ's oracle source and comparison rule are the first failing tests. Do not write a separate port-story Phase P or parity report.
