...
This directs the **auditor** agent to run a focused, lightweight review of a single story's **changed surface** as a soft gate between `/implement-story` and `/qa-story`.
...
- A story document at `docs/features/{STORY-ID}-*.md` (Sections 4b Ports & Adapters, 7 Files to CREATE/MODIFY, and 8a Test Plan are required).
- Optional: a `git diff` against the project's main branch — the auditor uses it to detect out-of-plan changes (files modified that are not listed in Section 7).
...
5. Writes findings to `docs/features/{STORY-ID}-review.md` using the review template. The **first line** of that file must be the single-line `REVIEW SUMMARY:` so QA and the Z6 gate can grep it.
6. Sets `Gate result` to `Block` if any finding is `severity: high` or `critical` (in any category). Otherwise `Pass`.
7. When `Block`, populates **Required actions before QA** with one bullet per blocking finding referencing the finding ID and the remediation step.

## Outputs

- `docs/features/{STORY-ID}-review.md` (new or overwritten).
- A short chat summary repeating the `REVIEW SUMMARY:` line and the gate result.
...
/implement-story  →  /review-story  →  /qa-story
                        |
                        ├─ Pass  → /qa-story
                        └─ Block → fix the listed actions, then re-run /review-story
```

`Z6` in the story's Phase Z requires this review to report zero `high`/`critical` findings on the changed surface.

## Examples

- `/review-story OTO-2` — review story OTO-2's changed surface

This command is available in chat with `/review-story`.
It expects one argument, the story ID, like so: `/review-story OTO-2`.
